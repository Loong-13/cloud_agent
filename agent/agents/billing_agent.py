...
def normalize_mcp_server_cwd(servers_config: dict[str, Any]) -> dict[str, Any]:
    """Resolve MCP subprocess settings from the active Python environment."""
    agent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    project_dir = os.path.dirname(agent_dir)
    for server_config in servers_config.get("mcpServers", {}).values():
        cwd = server_config.get("cwd")
        if cwd and not os.path.isabs(cwd):
            server_config["cwd"] = os.path.abspath(os.path.join(project_dir, cwd))
...
class BillingAgentNode:
    ...
    def __init__(self):
        ...
        normalize_mcp_server_cwd(self.servers_config)

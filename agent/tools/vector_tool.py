...
def _fallback_graph_keyword_search(query: str) -> str:
    ...
    for k in keywords:
        where_clauses.append(
            f"toLower(coalesce(n.id, '')) CONTAINS '{k}' OR toLower(coalesce(n.name, '')) CONTAINS '{k}' OR toLower(coalesce(n.description, '')) CONTAINS '{k}'")
    ...
    for k in keywords:
        rel_where_clauses.append(
            f"toLower(coalesce(a.id, '')) CONTAINS '{k}' OR toLower(coalesce(a.name, '')) CONTAINS '{k}' OR toLower(coalesce(b.id, '')) CONTAINS '{k}' OR toLower(coalesce(b.name, '')) CONTAINS '{k}'")
    ...
    if relations:
        ...
        for row in relations:
            ...
            parts.append(
                f"- [{from_labels}] {row.get('from_node', '')} -[{row.get('rel', '')}]-> [{to_labels}] {row.get('to_node', '')}")
    return "\n".join(parts)
